...
#> Fire Attack
execute unless score @s fireattacktime matches 0 run scoreboard players remove @s fireattacktime 1
# execute if entity @e[type=#models_logic:target, distance=..8] if score @s fireattacktime matches 0 as @e[distance=..1,tag=aj.bowser.root,limit=1,sort=nearest] run function animated_java:bowser/animations/animation.bowser.shoot/resume
execute if entity @e[type=#models_logic:target, distance=..8] if score @s fireattacktime matches 0 run function mtnt.main:bowser_shooting
